...
const SwiperLight = ({children, className}: SwiperProps) => {
    ...
    const endSwiping = () => {
        ...
        const width = Array.from(ref.current.children[0].children).reduce((acc, item) => acc + item.clientWidth, 0) + gap * (ref.current.children[0].children.length - 1)
    ...
    return (
        <div className={stack(styles.swiper, className)} onTouchStart={touchStart} onTouchEnd={endSwiping} onTouchMove={touchMove} onMouseDown={touchMouseStart}
